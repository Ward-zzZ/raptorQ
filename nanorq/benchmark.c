...
#define TEST_BYTES 1024 * 1024 * 1024 //256MB
              ...
              uint64_t *oti_common, uint32_t *oti_scheme, bool precalc)
{
  ...
  *oti_scheme = nanorq_oti_scheme_specific(rq);
  ...
  {
    uint64_t t0 = usecs();
    //这里重复编码，直到编码的数据>TEST_BYTES，以便测出吞吐量
              ...
              symvec *packets)
{
  ...
  int num_sbn = nanorq_blocks(rq);
  int num_packets = kv_size(*packets); //传输的总共packet数
  int packet_size = nanorq_symbol_size(rq);
  size_t bytes = 0;
  double elapsed = 0.0;
  while (bytes < TEST_BYTES)
...
int main(int argc, char *argv[])
{
  ...
  float overhead = strtof(argv[3], NULL);         // overhead packets number
  run(num_packets, packet_size, overhead);        //一次发送一个block（packet_size和Al对齐的情况，否则为2个block）
